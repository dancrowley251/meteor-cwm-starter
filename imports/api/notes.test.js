...
if (Meteor.isServer) {
  describe('notes', function() {
    ...
    it('should throw error if extra updates', function() {
      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({
          ...
          noteOne._id,
          { title, malicousValue: 'haha' }
        ]);
      }).toThrow();
    });

    it('should not update note if the user was not creator', function() {
      const title = 'This is an updated title';
    ...
    it ('should return 0 notes that has no notes', function() {
      const res = Meteor.server.publish_handlers.notes.apply({ userId: 'somerandomvalue' });
      const notes = res.fetch();
